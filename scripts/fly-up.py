...
- Android: wallet-proxy serves /.well-known/assetlinks.json built from the
  same production app fingerprints helm-charts' wellknownAndroidPackage-
  NamesAndFingerprints carries, plus optionally a developer's own local
  debug keystore fingerprint (--android-package/--android-fingerprint, or
  auto-read from .env.android if present - see generate_android_assets()).
...
def render_configs(env: str, chart_dir: Path):
    run([sys.executable, "scripts/render-helm-config.py", "--target", "fly", "--env", env,
         "--chart-dir", str(chart_dir)], cwd=SIROSID_DEV_ROOT)
...
def generate_android_assets(docs: list, out_dir: Path, extra_fingerprint: tuple | None = None) -> Path:
    ...
    assetlinks_path.write_text(assetlinks_json(wellknown, extra_fingerprint=extra_fingerprint))
...
def debug_android_fingerprint(args) -> tuple | None:
    """--android-package/--android-fingerprint, or auto-read from .env.android
    (written by `make android-setup` - APK_KEY_HASH/ANDROID_PACKAGE) if
    present and neither flag was passed explicitly.
    """
    if args.android_package and args.android_fingerprint:
        return (args.android_package, args.android_fingerprint)
    if args.android_package or args.android_fingerprint:
        raise SystemExit("--android-package and --android-fingerprint must be given together")
    ...
    package, fingerprint = values.get("ANDROID_PACKAGE"), values.get("APK_KEY_HASH")
    if not package or not fingerprint:
        return None
    # APK_KEY_HASH in .env.android is base64url (WebAuthn origin form), not
    # the colon-separated SHA-256 hex assetlinks.json needs - convert.
    import base64
    raw = base64.urlsafe_b64decode(fingerprint + "=" * (-len(fingerprint) % 4))
    hex_fingerprint = ":".join(f"{b:02X}" for b in raw)
    print(f".env.android found - adding debug fingerprint for {package}")
    return (package, hex_fingerprint)
...
def main():
    ...
    parser.add_argument("--android-package",
                         help="Add a debug APK's own package+fingerprint to assetlinks.json "
                              "(must be given with --android-fingerprint). Auto-read from "
    ...
    render_configs(args.env, chart_dir)
    ...
    extra_fingerprint = debug_android_fingerprint(args)
    assetlinks_path = generate_android_assets(docs, out_dir, extra_fingerprint=extra_fingerprint)
