...
ARG VITE_WALLET_ENGINE_URL=http://localhost:8082
...
ARG VITE_TRANSPORT_PREFERENCE=wmp,websocket,http
ARG VITE_ALLOWED_TRANSPORTS=http,websocket,wmp
ARG VITE_WMP_RPC_URL=
ARG VITE_WMP_EVENTS_URL=

# Set them as environment variables for the build
ENV VITE_WALLET_BACKEND_URL=$VITE_WALLET_BACKEND_URL
ENV VITE_WALLET_ENGINE_URL=$VITE_WALLET_ENGINE_URL
...
ENV VITE_TRANSPORT_PREFERENCE=$VITE_TRANSPORT_PREFERENCE
ENV VITE_ALLOWED_TRANSPORTS=$VITE_ALLOWED_TRANSPORTS
ENV VITE_WMP_RPC_URL=$VITE_WMP_RPC_URL
ENV VITE_WMP_EVENTS_URL=$VITE_WMP_EVENTS_URL
